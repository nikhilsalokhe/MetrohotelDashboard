...
<div class="card m-2 border">
  <div class="card-header">
      @include('backbutton')
      <h2 class="text-center mb-2"> Mothly Bookings</h2>
  </div>
<div class="card-body">
    ...
    <tbody>
      ...
      <tr>
          <td>{{ $month['start']->format('F Y') }}</td>
          <td>{{ round($month['avgBill']) }}</td>
          <td>{{ round($month['sumDays']) }}/930</td>
          <td>{{ round(($month['sumDays'] / 930) * 100)}}%</td>
      </tr>
      @endforeach
  </tbody>
    </table>
